...
if(session.getAttribute("username")!=null && session.getAttribute("password")!=null)
{
	...
	String sql = "SELECT * from exer_cal where account = '"+name1+"' and date_format(date,'%Y-%m')='"+date1+"' order by exer_cal_id";
...
if(count>1){
	...
	String[] series = {"基礎代謝率", "運動消耗熱量", "總消耗熱量"};
	
	for(int i=0;i<count;i++)
	{
		String date2=rs.getString("date");
		data.addValue(bmr, series[0],date2.substring(8));
		rs.next();
	}
	...
	{
		String date2=rs.getString("date");
		data.addValue(bmr+rs.getInt("cal"), series[2], date2.substring(8));
		rs.next();
	}
	...
	JFreeChart chart =
	ChartFactory.createLineChart(Integer.parseInt(month.substring(5,7))+"月份運動熱量消耗統計圖",
	                               "日期",
	                               "熱量(大卡)",
	                               data,
...
<div style="background:url(images/index/background.jpg) repeat; margin-top:2px; padding-top:20px">
	<div style="width:990px; height:1180px; margin:0 auto 0 auto">
		<div style="width:988px; height:13px; background:url(images/index/inside_top.gif) no-repeat;">
		</div>
		<div style="position:relative; width:988px; height:1080px; background:url(images/index/inside_bg.gif) repeat-y;">
			<img style="position:absolute; top:444px; left:36px;" src="images/resume/illustrate_1.png">
			...
			<div style="position:absolute; top:12px; right:36px;">
				<div style="width:676px; height:48px; background:url(images/resume/exer_title.png) no-repeat;"></div>
					<div style="width:676px; height:15px; background:url(images/resume/foodcal_in_title.png) no-repeat;"></div>
					<div style="width:676px; height:515px; background:url(images/resume/foodcal_in_middle.png) repeat-y;">
    ...
    <td><table width="600" border="0" cellspacing="0" cellpadding="0">
      <tr>
       ...
       %>
        ...
        <td width="420" align="center" style="color:#4E96DE"><h3><%=iYear%>年&nbsp;-&nbsp;<%=new SimpleDateFormat("MMMM").format(new java.util.Date(2008,iMonth,01))%>&nbsp;-&nbsp;運動熱量消耗日曆</h3></td>
        <td width="60">月份&nbsp;</td>
        <td width="42">
            ...
            for(int j=1;j<=7;j++)
            {
               ...
               <% 
                ...
                String date2 = "";
             	...
             	String sql2 = "SELECT exer_cal_id, cal from exer_cal where account = '"+name1+"' and date_format(date,'%Y-%m-%d')='"+date2+"'";
            	...
            	%>
            		<img src="images/resume/search_exer.png">
            	<%	
            	}
            	else{
            		out.print("<div style='color:red'>"+rs2.getString("cal")+"大卡</div>");
            		exer_cal_id = rs2.getInt("exer_cal_id");
            	%>
            		<img src="images/resume/search_exer.png" onclick="window.open ('exer_data.jsp?exer_cal_id=<%=exer_cal_id%>', 'newwindow', 'height=265, width=900, top=0, left=0, toolbar=0, menubar=0, scrollbars=1, resizable=0, location=0, status=0')">
            	<%	
            	}
                %>                  		
                </td>
