...
if(request.getParameter("name")!=null){
	...
	String sql1 = "SELECT * from eat_cal where account = '"+name1+"' and date = '"+date+"'";
...
<div style="position:relative; margin-top:20px; width:593px; height:368px; background:url(images/resume/dialog.png) no-repeat;">
	<form action="food_to_resume.jsp" method="post" name="add_resume">
		<input type="hidden" name="name5" value="<%=name1 %>">
		<input type="hidden" name="totca5" value="<%=totca %>">
		<input type="hidden" name="date5" value="<%=date %>">
		<input type="hidden" name="eat_cal_id" value="<%=rs1.getInt("eat_cal_id")%>">
<%
for (int i = 0; i < food.length; i++){
%>		
		<input type="hidden" name="food" value="<%=food[i]%>">
...
String sql = "INSERT INTO eat_cal(cal, date, account) VALUES('"+totca+"','"+date+"','"+name1+"')";
smt.execute(sql);
String sql2 = "SELECT eat_cal_id from eat_cal where account = '"+name1+"' and date = '"+date+"'";
ResultSet rs2 = smt.executeQuery(sql2);
rs2.first();
int eat_cal_id = rs2.getInt("eat_cal_id");
for (int i = 0; i < food.length; i++){
String sql3 = "INSERT INTO food_record(food_name, amount, eat_cal_id) VALUES('"+food[i]+"','"+amount[i]+"','"+eat_cal_id+"')";
smt.execute(sql3);
}
%>
<div style="position:relative; margin-top:20px; width:593px; height:368px; background:url(images/resume/save_succ.png) no-repeat;">
	...
	<meta http-equiv="refresh" content="3;url=resume7.jsp">
</div>
<%
}
}
if(request.getParameter("name5")!=null){
	...
	String[] food=request.getParameterValues("food");
	String[] amount=request.getParameterValues("amount");
	String sql = "UPDATE eat_cal SET cal = '"+totca5+"' WHERE date = '"+date5+"' and account = '"+name5+"'";
	smt.executeUpdate(sql);
	String sql1="delete from food_record where eat_cal_id='"+eat_cal_id+"'";
